#====================================================================
# EKS MODULE
# Main configuration for EKS Cluster with Istio Service Mesh
# Components: EKS, Istio, Ingress Gateway, Egress Gateway, Strapi
#====================================================================
...
resource "aws_eks_cluster" "main" {
  ...
  vpc_config {
    ...
    public_access_cidrs     = var.eks_public_access ? var.eks_public_access_cidrs : []
...
# Custom policy for nodes to access KMS, Secrets Manager, etc.
...
resource "aws_security_group" "eks_nodes" {
  ...
  ingress {
    description              = "Allow cluster control plane to communicate with nodes (443)"
    from_port                = 443
    to_port                  = 443
    protocol                 = "tcp"
    source_security_group_id = aws_security_group.eks_cluster.id
...
# System Node Group (for Istio, CoreDNS, etc.)
resource "aws_eks_node_group" "system" {
  ...
  tags = merge(var.tags, {
    Name = "${var.project_name}-${var.environment}-eks-system-nodes"
  })
...
resource "aws_eks_node_group" "application" {
  ...
  tags = merge(var.tags, {
    Name = "${var.project_name}-${var.environment}-eks-app-nodes"
  })
...
#====================================================================
# ISTIO SERVICE ACCOUNT IAM ROLE (IRSA)
#====================================================================

resource "aws_iam_role" "istio_ingress" {
  name = "${var.project_name}-${var.environment}-istio-ingress-role"
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${replace(aws_iam_openid_connect_provider.eks.url, "https://", "")}:aud" = "sts.amazonaws.com"
            "${replace(aws_iam_openid_connect_provider.eks.url, "https://", "")}:sub" = "system:serviceaccount:istio-system:istio-ingressgateway"
          }
        }
      }
    ]
...
resource "aws_iam_role" "istio_egress" {
  name = "${var.project_name}-${var.environment}-istio-egress-role"
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${replace(aws_iam_openid_connect_provider.eks.url, "https://", "")}:aud" = "sts.amazonaws.com"
            "${replace(aws_iam_openid_connect_provider.eks.url, "https://", "")}:sub" = "system:serviceaccount:istio-system:istio-egressgateway"
          }
        }
      }
    ]
...
#====================================================================
# STRAPI SERVICE ACCOUNT IAM ROLE (IRSA)
#====================================================================

resource "aws_iam_role" "strapi" {
  name = "${var.project_name}-${var.environment}-strapi-role"
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${replace(aws_iam_openid_connect_provider.eks.url, "https://", "")}:aud" = "sts.amazonaws.com"
            "${replace(aws_iam_openid_connect_provider.eks.url, "https://", "")}:sub" = "system:serviceaccount:strapi:strapi"
          }
        }
      }
    ]
...
resource "aws_iam_role_policy" "strapi" {
  name = "${var.project_name}-${var.environment}-strapi-policy"
  role = aws_iam_role.strapi.id
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "arn:${data.aws_partition.current.partition}:secretsmanager:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:secret:${var.project_name}/${var.environment}/strapi/*"
      },
      {
        Effect = "Allow"
        Action = [
